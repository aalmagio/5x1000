...
> **Nota sul RUNTS**: la cartella `Runts/` è inclusa nel repo con l'ultima versione disponibile del file. Quando il RUNTS si aggiorna, scarica il nuovo `.xlsx` da [registroterzosettore.gov.it](https://www.registroterzosettore.gov.it), sostituisci il file in `Runts/` e committa. Se nella cartella ci sono più file `.xlsx` viene usato l'ultimo in ordine alfabetico. Il join avviene su `COD_FISCALE`; le colonne rilevanti (sezione, sede, data iscrizione, abilitazione 5x1000) vengono riconosciute automaticamente per nome. Il match attuale è circa il 55% delle righe (gli enti più vecchi non sono ancora nel RUNTS). Per saltare il merge: `python etl.py --no-runts`.
...
│
├── Runts/                # File RUNTS .xlsx (in repo; aggiornare periodicamente da registroterzosettore.gov.it)
